...
// --------------------------------------------------
// ⭐ NEW: PANEL MOVEMENT + FULLSCREEN SYSTEM
// --------------------------------------------------
const panelA = document.getElementById("panelA");
const panelB = document.getElementById("panelB");
const mainLayout = document.querySelector(".movable-layout");

// Swap panels left/right
document.querySelectorAll(".panel-move").forEach(btn => {
  btn.addEventListener("click", () => {
    if (panelA.nextElementSibling === panelB) {
      mainLayout.insertBefore(panelB, panelA);
    } else {
      mainLayout.insertBefore(panelA, panelB);
    }
  });
});

// Fullscreen toggle
document.querySelectorAll(".panel-fullscreen").forEach(btn => {
  btn.addEventListener("click", () => {
    const target = document.getElementById(btn.dataset.target);
    const isFull = target.classList.contains("fullscreen");

    // Remove fullscreen from any panel
    document.querySelectorAll(".panel.fullscreen")
      .forEach(p => p.classList.remove("fullscreen"));

    // Toggle fullscreen on this one
    if (!isFull) target.classList.add("fullscreen");
  });
});
